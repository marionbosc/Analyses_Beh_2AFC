...
%  - DataLater: 1 month after lesion dataset pathway
...
function CRI2(DataBefore, DataAfter, DataLater, Modality, nbBin,perside)
...
for bin = 1: size(idx_DV_Before,2)
    ...
    subplot(3,nbBin/perside,bin); hold on;
    % Essais Error WS 
...
for bin = 1 : size(idx_DV_After,2)
    ...
    subplot(3,nbBin/perside,bin+nbBin); hold on;
    % Essais Error WS 
...
%% Chargement data Later:
% Chargement datas 1 mois apres lesion:
load(DataLater);
SessionData = SessionDataWeek;
...
if Modality ==1
    ...
    idx_DV_Later = unique(BinIdx(~isnan(BinIdx)));
    for bin = 1: size(idx_DV_Later,2)
        if sum(SessionData.Custom.CatchTrial==1 & BinIdx==idx_DV_Later(bin)) < 10
            idx_DV_Later(bin) =  NaN;
        end
    end
    idx_DV_Later = idx_DV_Later(~isnan(idx_DV_Later));     
    nbBin = size (idx_DV_Later,2);
else
    if perside ==2
        DV = SessionData.Custom.DVlog(1:numel(SessionData.Custom.ChoiceLeft));
        % Calculs Bins de difficulte
        % Recup des bornes des bins par percentile de la distrib reelle des essais
        DV_pctile_Later = min(DV);
        for j = 1:nbBin
            DV_pctile_Later = [DV_pctile_Later prctile(DV,(100/nbBin)*j)];
        end
        %BinIdx = discretize(DV,linspace(-1.6,1.6,nbBin+1));
        BinIdx = discretize(DV,DV_pctile_Later);
    else
        DV = abs(SessionData.Custom.DVlog(1:numel(SessionData.Custom.ChoiceLeft)));
        % Calculs Bins de difficulte
        % Recup des bornes des bins par percentile de la distrib reelle des essais
...
for bin = 1 : size(idx_DV_Later,2)
    ...
    if Modality ==2
        % Recup data plot distrib donnee par bin de DV:
        histog_DV.Later.Catch{bin} = SessionData.Custom.DVlog(ndxCatch & ndxModality & BinIdx==idx_DV_Later(bin));
        histog_DV.Later.Error{bin} = SessionData.Custom.DVlog(ndxFalse & ndxModality & BinIdx==idx_DV_Later(bin));
    end
    Perf_per_bin.Later(bin) = sum(ndxCorrect& BinIdx==idx_DV_Later(bin)& ndxModality)/...
        sum(ndxCorrect& BinIdx==idx_DV_Later(bin)& ndxModality|ndxFalse & BinIdx==idx_DV_Later(bin)& ndxModality);
    
    subplot(3,nbBin/perside,bin+(2*nbBin)); hold on;
    % Essais Error WS 
    D = histogram(SessionData.Custom.FeedbackTimeNorm(ndxFalse & BinIdx==idx_DV_Later(bin) & ndxModality),...
        'FaceColor','m','EdgeColor','m','BinWidth',0.1,'Normalization','probability'); hold on; %
    D.FaceAlpha=0.3;
    % Recup data
    Data_Error_WT{bin} = D.BinEdges(2:end);
    ...
    % Essais Correct Catched
    F = histogram(SessionData.Custom.FeedbackTimeNorm(ndxCorrect & ndxCatch & BinIdx==idx_DV_Later(bin) & ndxModality),...
        'FaceColor','y','EdgeColor','y','BinWidth',0.1,'Normalization','probability'); hold on; %
    F.FaceAlpha=0.3;
    % Recup data
    Data_Catch_WT{bin} = F.BinEdges(2:end);
    ...
    i=1; 
    for Theta = bin_WT
        P_Error_Later{bin}(i) = sum(Data_Error_P{bin}(Data_Error_WT{bin}>Theta));
        P_Catch_Later{bin}(i) = sum(Data_Catch_P{bin}(Data_Catch_WT{bin}>Theta)); 
        i=i+1;
    end
    
    AUC_Later(bin)= trapz(P_Error_Later{bin},P_Catch_Later{bin});  
    
    if Modality ==1
        title(['LATER: DV = ' num2str(idx_DV_Later(bin))  ' / AUC = ' num2str(AUC_Later(bin))],...
            'fontsize',10);
    else
        title(['LATER: DV = ' num2str(round(DV_pctile_Later(bin),2)) ' : ' num2str(round(DV_pctile_Later(bin+1),2)) ' / AUC = ' num2str(AUC_Later(bin))],...
             'fontsize',10);
    end
...
figtitle(['P(WT) per DV percentile BEFORE / AFTER lesion and LATER ' SessionData.Custom.Subject '   '  SessionData.SessionDate],'fontsize',14,'fontweight','bold');
...
if Modality == 2
    ...
    for bin = 1 : size(idx_DV_Later,2)
        h=histogram(histog_DV.Later.Catch{bin},'BinWidth',0.02); 
        hold on
        if perside ==2
            legend_name{bin} = [num2str((100/nbBin)*bin) ' th DV pctl = ' num2str(round(min(h.Data),2)) ' : ' num2str(round(max(h.Data),2))];
        else
    ...
    xlabel('-log(DV)','fontsize',14);ylabel('Number of trials','fontsize',14); xlim([-1.7 1.7]);
    %legend(legend_name,'Location','North');
    title('Correct Catch trials LATER',...
            'fontsize',12);

    subplot(3,2,6);
    for bin = 1 : size(idx_DV_Later,2)
        h = histogram(histog_DV.Later.Error{bin},'BinWidth',0.02); 
        hold on
    %     if perside ==2
    %         legend_name{bin} = [num2str((100/nbBin)*bin) ' th DV pctl = ' num2str(round(min(h.Data),2)) ' : ' num2str(round(max(h.Data),2))];
    %     else
    ...
    xlabel('-log(DV)','fontsize',14);ylabel('Number of trials','fontsize',14); xlim([-1.7 1.7]);
    legend(legend_name,'Location','NorthWest');
    title('Error trials LATER',...
            'fontsize',12);

    figtitle(['Distribution of trials per DV percentile BEFORE / AFTER lesion and LATER ' SessionData.Custom.Subject '   '  SessionData.SessionDate],'fontsize',14,'fontweight','bold');
end
%% Plot AUC Before-After
figure('units','normalized','position',[0,0,1,1]); hold on

if Modality ==1
    idx_DV = unique([idx_DV_Before idx_DV_After idx_DV_Later]);
    for bin = 1: size(idx_DV,2)
        ...
        idx = find(idx_DV_Later == idx_DV(bin));
        if ~isempty(idx)
            plot(P_Error_Later{idx},P_Catch_Later{idx},'c','Linewidth',1);
            later =1;
        else
            later = 0;
        end
        ...
        if before ==1
            if  after ==1
                if later ==1
                    legend('Before','After','Later','Location','SouthEast');ylim([0 1]);
                else
                    legend('Before','After','Location','SouthEast');ylim([0 1]);
                end
            elseif later ==1
                legend('Before', 'Later','Location','SouthEast');ylim([0 1]);
            else
                legend('Before', 'Location','SouthEast');ylim([0 1]);
            end
        elseif after ==1  
            if later ==1
                legend('After','Later','Location','SouthEast');ylim([0 1]);
            else 
                legend('After','Location','SouthEast');ylim([0 1]);
            end
        else
            legend('Later','Location','SouthEast');ylim([0 1]);
        end
    end
else
    for bin = 1: size(idx_DV_After,2)
        ...
        plot(P_Error_Later{bin},P_Catch_Later{bin},'c','Linewidth',1);       
        plot([0 1],[0 1],'--k');
...
AUCnorm_Later = (AUC_Later - min(AUC_Later))/(1 -min(AUC_Later)) ;
if Modality ==1
    plot(idx_DV_Before,AUCnorm_before,'-+k','Linewidth',1);
    plot(idx_DV_After,AUCnorm_After,'-+r','Linewidth',1);
    plot(idx_DV_Later,AUCnorm_Later,'-+c','Linewidth',1);
else
    plot(DV_pctile_Before(1:end-1),AUCnorm_before,'-+k','Linewidth',1);
    plot(DV_pctile_After(1:end-1),AUCnorm_After,'-+r','Linewidth',1);
    plot(DV_pctile_Later(1:end-1),AUCnorm_Later,'-+c','Linewidth',1);
end
xlabel('DV','fontsize',14);ylabel('CRI (rescaled)','fontsize',14);%ylim([0 1]);
legend('BEFORE','AFTER','LATER','Location','Northwest');
title('Lesion impact on Confidence Report Index',...
        'fontsize',12);

figtitle(['Confidence Report Index BEFORE / AFTER lesion and LATER ' SessionData.Custom.Subject],'fontsize',14,'fontweight','bold');
...
plot(Perf_per_bin.Later,AUC_After,'-+c','Linewidth',1);
xlabel('Success Rate','fontsize',14);ylabel('CRI','fontsize',14);%ylim([0 1]);
legend('BEFORE','AFTER','LATER','Location','Northwest');
title('Lesion impact on Confidence Report Index',...
        'fontsize',12);

figtitle(['Confidence Report Index BEFORE / AFTER lesion and LATER ' SessionData.Custom.Subject '  ' SessionData.SessionDate],'fontsize',14,'fontweight','bold');